...
(function () {
  ...
  // If this file is ever included twice (a duplicated <script> tag, or a copy
  // pulled in by something else), the whole builder set below would be rebuilt
  // and window.Factory replaced. Every geometry this file caches session-wide
  // since V13.5.0 would be orphaned but still referenced by objects already in
  // the scene, so both copies would be pinned in memory for the rest of the
  // session. Second and later loads now stop right here.
  if (window.Factory && window.Factory.__drddLoaded) {
    try { console.warn('[Factory.js] already loaded — this second copy was ignored (check for a duplicate <script> tag).'); } catch (e) {}
    return;
  }
...
// (characterDipeGenie draft removed V13.7.0 — superseded by the live design below)
  ...
  window.Factory = {
    __drddLoaded: true,   // v23.1.0 — what the double-load guard at the top reads
    // ── Characters ──────────────────────────────────────────────────────────

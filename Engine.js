...
(function () {
  ...
  // A second copy of this file would replace window.Engine with a fresh, un-init
  // instance while the game holds the old one, and would allocate a second set
  // of render targets and PMREM textures that nothing would ever dispose.
  if (window.Engine && window.Engine.__drddLoaded) {
    try { console.warn('[Engine.js] already loaded — this second copy was ignored (check for a duplicate <script> tag).'); } catch (e) {}
    return;
  }
  ...
  E.__drddLoaded = true;    // v23.1.0 — what the double-load guard at the top reads
  window.Engine = E;

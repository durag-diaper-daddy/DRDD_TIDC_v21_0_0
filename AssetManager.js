...
(function () {
  ...
  var MANIFEST = {
    ...
    enemyJoeLBossHead:       { file: 'joel_head.glb',     height: 4.5, rotZ:90 }
  };
  ...
  A.boot = function () {
    ...
    bootstrapLoader().then(function (boot) {
      ...
      function _dlLaunch(job) {
        ...
        (function attempt(n) {
          loader.load(PATH + job.file, function (gltf) { freeSlot(); job.res(gltf); }, undefined, function (err) {
            ...
            if (n < 2) {
              diag('warn', job.file + ' failed to fetch (attempt ' + (n + 1) + ' of 3) — retrying...');

...
(function () {
  ...
  var MANIFEST = {
    ...
    // THE LADDER: v23.6.0 rotZ:90 (no visual check). v23.13.0: the AUTO
    // stand-up applied rotX:+90 → owner screenshot: FACE-DOWN. v23.14.0:
    // explicit rotX:-90 → owner screenshot: ON THE BACK OF HIS HEAD.
    // Those two results are a mirrored pitch pair, 180° of face travel —
    // which means the delivered model is UPRIGHT and every quarter turn
    // was a wrong guess (the bbox auto fired only because a head is
    // near-spherical and measures ambiguous). v23.15.0: rotX:0 — explicit
    // zero, which now suppresses the auto (the != null fix above) and
    // applies nothing. rotZ:90 kept: one variable per rung. If he lands
    // ear-down next, the rung after this drops rotZ.
    enemyJoeLBossHead:       { file: 'joel_head.glb',     height: 4.5, rotX:0, rotZ:90 }
  };
  ...
  A.progress = function () {
    return {
      ...
      waiting: _prog.wait ? Object.keys(_prog.wait) : [],
  ...
  A.boot = function () {
    ...
    bootstrapLoader().then(function (boot) {
      ...
      function settleOne(wasOk, key) {
        _prog.done++; if (wasOk) _prog.ok++; else _prog.miss++;
      ...
      function loadFile(file, urgent) {
        ...
        filePromises[file] = new Promise(function (res, rej) {
          ...
          // urgent (wave 2) launches NOW, above the limit — see the note on
          // the queue above. Everything else waits its turn for a slot.
          if (urgent) _dlLaunch(job); else { _dlQueue.push(job); _dlPump(); }
        });
